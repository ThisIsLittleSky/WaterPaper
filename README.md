...
| 降AI | 硬着头皮改套话、调句式 | D0-D6 六维约束 + 30+ AI 模式扫描 + 三级词汇清洗 |
...
│   ├── humanize_constraints.md       #   D0-D6 降AI写作约束（整合 thesis-optimizer）
│   ├── humanize_pass.md              #   独立降AI改写流程（含模式扫描 + 三维改写）
│   └── plagiarism_pass.md            #   [新增] 独立降重改写流程
│
├── tools/                            # Python 工具脚本
...
│   └── humanize_check.py             #   降AI效果验证（含三级词汇报告 + 术语保护 + 密度检查）
│
└── references/                       # 参考规范
    ...
    ├── ai_pattern_taxonomy.md        #   [新增] 30+ AI 模式分类学（整合 thesis-optimizer）
    ├── ai_vocabulary_blacklist.md    #   [新增] 三级词汇黑名单（整合 thesis-optimizer）
    └── term_whitelist.md             #   [新增] 术语保护白名单
```
...
- 写作完成后自动运行 `humanize_check.py` 验证，不通过不交付
