...
12. 写作阶段必须应用 D1-D5 降AI约束（参考 `prompts/humanize_constraints.md`，默认 medium 档），不得使用禁用的 AI 高频连接词和套话。
13. 成稿前必须运行 `python tools/humanize_check.py <paper.md> --markdown` 验证，句长标准差 ≥ 6 且连接词密度 ≤ 8/千字 才能交付。
...
```
intake → format_confirmed → topic_selection → topic_confirmed → literature_collected → outline_confirmed → writing → humanize_check → delivery
```
...
- 未通过 `humanize_check` 前，禁止进入 `delivery`
...
2. 对问题段落使用 `prompts/humanize_pass.md` 的改写流程
3. 重新运行检查直到通过
...
降AI写作约束（详见 `prompts/humanize_constraints.md`，默认 medium 档）：

1. **D1 句长**：每 3-4 句必须有一句长度显著偏离（≤10字 或 ≥35字），禁止连续 3 句长度差 < 8 字。
2. **D2 段落结构**：全文使用至少 4 种段落模板（提问驱动/对比判断/因果链/点题收束/正反综合），相邻段落不同模板。
3. **D3 信息密度**：核心段高密度（配具体证据）+ 过渡段低密度（1-2 句过渡），形成"高-低-高"交替。
4. **D4 连接词**：禁用"首先/其次/最后""综上所述""此外""值得注意的是"等 AI 高频词，每千字连接词 ≤ 6 个。
5. **D5 术语**（heavy 档）：偶尔使用同义学术表达替代标准术语。
6. **禁用套话**："具有重要意义""为……奠定基础""在……的过程中""随着……的快速发展"。
7. **禁止长横线**：如 "————"，用空白行或章节标题替代。
8. 正文不用"本文""笔者"开篇太多，适当变化句式。
9. 每个观点尽量配文献支撑，不写无出处断言。
10. 致谢可自然一些，但正文不能口语化。
...
### Humanize（降AI）
- `prompts/humanize_constraints.md` — D1-D5 降AI写作约束
- `prompts/humanize_pass.md` — 独立降AI改写 Prompt
- `tools/humanize_check.py` — 降AI效果验证脚本
